      ...
      <div className="max-w-4xl mx-auto p-4 pb-20">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="p-6 md:p-8">
            {/* Header */}
            <div className="grid grid-cols-1 md:grid-cols-[70%_30%] gap-4 mb-8 pb-6 border-b-2" style={{ borderColor: brandColor }}>
              <div className="flex gap-4 items-center">
                ...
                <div className="flex-1 flex flex-col justify-center">
                  ...
                  <div className="text-sm text-gray-600">
                    ...
                    <p>WhatsApp: {storeSettings?.whatsapp || '+62 XXX XXX XXX'}</p>
                  </div>
                </div>
              </div>
              <div className="text-left md:text-right mr-4 flex flex-col justify-center">
                <h1 className="text-4xl font-bold mb-0 leading-tight" style={{ color: brandColor }}>INVOICE</h1>
            ...
            <div className="mb-8 overflow-x-auto rounded-lg">
              <table className="w-full ">
                <thead className="text-white " style={{ backgroundColor: brandColor }}>
                  <tr>
                    <th className="px-3 py-3 text-left text-xs font-semibold uppercase">No</th>
                    <th className="px-3 py-3 text-left text-xs font-semibold uppercase">Description</th>
                    <th className="px-3 py-3 text-right text-xs font-semibold uppercase hidden sm:table-cell">Qty</th>
                    <th className="px-3 py-3 text-right text-xs font-semibold uppercase hidden sm:table-cell">Price</th>
                ...
                <tbody className='border-b'>
                  {items.map((item, index) => (
                    <tr key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} >
                      ...
                      <td className="px-3 py-3 text-right text-sm hidden sm:table-cell">{item.quantity}</td>
                      <td className="px-3 py-3 text-right text-sm hidden sm:table-cell">{formatCurrency(item.price)}</td>
